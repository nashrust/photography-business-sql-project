...
SELECT *
FROM CLEAN_PACKAGE
WHERE FINAL_CLEANED_PRICE = '$0.00';

SELECT *
FROM CLEAN_CLIENTS
WHERE CLEAN_EMAIL IS NULL;

SELECT *
FROM CLEAN_PACKAGE
WHERE FINAL_CLEANED_DURATION = 0;

SELECT *
FROM CLEAN_CLIENTS
WHERE CLEAN_FORMATTED_PHONE IS NULL;
...
SELECT CLEAN_FORMATTED_PHONE,
       COUNT(*)
FROM CLEAN_CLIENTS
GROUP BY CLEAN_FORMATTED_PHONE
HAVING COUNT(*) > 1;
...
FROM CLEAN_PACKAGE
WHERE
      (FINAL_PACKAGE_TIER='Basic'    AND FINAL_CLEANED_DURATION<>45)
   OR (FINAL_PACKAGE_TIER='Standard' AND FINAL_CLEANED_DURATION<>60)
   OR (FINAL_PACKAGE_TIER='Premium'  AND FINAL_CLEANED_DURATION<>120)
   OR (FINAL_PACKAGE_TIER='Deluxe'   AND FINAL_CLEANED_DURATION<>240)
   OR (FINAL_PACKAGE_TIER='Luxury'   AND FINAL_CLEANED_DURATION<>480);

SELECT *
FROM CLEAN_PACKAGE
WHERE
      (FINAL_PACKAGE_TIER='Basic'    AND FINAL_CLEANED_PRICE<>'$150.00')
   OR (FINAL_PACKAGE_TIER='Standard' AND FINAL_CLEANED_PRICE<>'$199.99')
   OR (FINAL_PACKAGE_TIER='Premium'  AND FINAL_CLEANED_PRICE<>'$299.99')
   OR (FINAL_PACKAGE_TIER='Deluxe'   AND FINAL_CLEANED_PRICE<>'$599.99')
   OR (FINAL_PACKAGE_TIER='Luxury'   AND FINAL_CLEANED_PRICE<>'$2999.99');
...
SELECT *
FROM CLEAN_CLIENTS
WHERE LENGTH(CLEAN_FORMATTED_PHONE) <> 10;

SELECT *
FROM CLEAN_CLIENTS
WHERE CLEAN_EMAIL NOT LIKE '%@%.%';

==========================
7. Summary Statistics
==========================

SELECT FINAL_PACKAGE_TIER,
       COUNT(*) AS TOTAL_PACKAGES
FROM CLEAN_PACKAGE
GROUP BY FINAL_PACKAGE_TIER
